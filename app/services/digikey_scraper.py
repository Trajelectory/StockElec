...
def _get_token(client_id: str, client_secret: str) -> str | None:
    ...
    try:
        ...
        if token:
            ...
            return token
        # Ne pas logger `data` en entier — peut contenir client_secret en echo
        err_msg = data.get("error_description") or data.get("error") or "réponse invalide"
        logger.warning("[DigiKey] Pas de token dans la réponse : %s", err_msg)
